...
use defmt::println;
use stm32f4xx_hal::{timer::{Pins, CCR, PwmExt, Pwm, Ch}, dma::MemoryToPeripheral,
...
pub use dma_timer::DmaCcrTimer;
use stm32f4xx_hal::timer::PwmHz;
...
//        let break_length = 10_000 / 1_250;
        let break_length = 16;   // can fail when run on a platform where usize is smaller than 32-bits. For example, some microcontrollers use 16-bit integers as the native size:
//        let stop_length: u32 = break_length as u32;
          ...
          <CCR<TIM, TIM_CHAN> as PeriAddress>::MemSize: From<u16> + Word + 'static + Copy
{
    ...
    fn write<T, I>(&mut self, iterator: T) -> Result<(), Self::Error>
        ...
        let buf = {
            ...
            let wait = (0..=( self.break_length )).map(|_| 0.into() ); // 0.into() func here
//            let end  = (0..=( self.break_length - 1 )).map(|_| 0.into() );
